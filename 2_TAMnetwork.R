...
Marker_genes=read.csv("humanTAM.csv")
Marker_genes=as.character(Marker_genes[,2])

Marker_genes = Reduce(intersect, list(Marker_genes,rownames(Gene_CGGALGG),rownames(Gene_LGG)))  # 243
...
Gene_LGG=Gene_LGG[Marker_genes,]  
dim(Gene_LGG)   # 243  525
...
###
Gene_CGGALGG=Gene_CGGALGG[Marker_genes,]  
dim(Gene_CGGALGG)   # 243  172
...
dev.new()
pdf(file="./Results220617/TAM_scaleFree.pdf",width=5,height=5)
sizeGrWindow(10,10)
par(mfrow=c(1,2))
hist(k)
...
ggsave("./Results220617/TAM_scaleFree.pdf", plot=Plot, width = 5, height = 5)
...
#colorDynamicHybridTOM (beta=5, deepSplit = 3, minClusterSize = 25)
...
write.csv(geneSet_1, file = "TAMgeneSet_1.csv")
write.csv(geneSet_2, file = "TAMgeneSet_2.csv")
write.csv(geneSet_3, file = "TAMgeneSet_3.csv")
write.csv(geneSet_4, file = "TAMgeneSet_4.csv")
...
ggboxplot(dataset1, x='Gene Module', y='K-index for overall survival', fill='Gene Module', 
  ...
  stat_compare_means(method = "anova", label.y = 0.73)+
